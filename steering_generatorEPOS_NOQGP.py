...
if len(sys.argv)>1:
  ...
  ########
  output = open("/corsikaqgp/steering/"+str(date2)+'_NOQGP_EPOS.steering','w+')
  #output.write("DIRECT  /Corsika/showerLib/"+str(date2)+"_EPOS_\n")    
  output.write("DIRECT  ./particles/output/"+str(angle)+"deg_"+str(primary)+"prm_"+str(energy)+"GeV_"+str(date2)+"_EPOS_NOQGP_\n")     
  ...
  output.write("EPOPAR input ../epos/eposnoqgp.param        !initialization input file for epos\n")
  ...
  print 'Usage:'
  print './corsika74000Linux_EPOS_fluka < /corsikaqgp/steering/'+str(date2)+'_NQQGP_EPOS.steering > /corsikaqgp/steeringEPOS_'+str(date2)+'_NOQGP.out'
