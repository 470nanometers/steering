...
if len(sys.argv)>1:
  ...
  ########
  output = open("/corsikaqgp/steering/"+str(date2)+'_'+'QGSJETII.steering','w+')
  output.write("DIRECT  ./particles/output/"+str(angle)+"deg_"+str(primary)+"prm_"+str(energy)+"GeV_"+str(date2)+"_QGSJET_               output directory\n")    
  ...
  ### QGSJET ###
  output.write("QGSJET  T  0                      use QGS for high energy hadrons\n")
  output.write("QGSSIG  T                         use QGS hadronic cross sections\n")
  #####
  ...
  print 'Usage:'
  print './corsika74000Linux_QGSII_fluka < /corsikaqgp/steering/'+str(date2)+'_'+'QGSJETII.steering > /corsikaqgp/'+str(date2)+'_'+'QGSJETII.out'
...
#### if the file is not input ####
  ...
  print "Usage: python steering_generatorQGSJET.py energy thin wmax rmax"
  print "Energy in GeV"
